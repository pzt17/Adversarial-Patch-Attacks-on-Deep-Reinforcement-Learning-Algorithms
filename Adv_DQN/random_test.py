...
def mstate_patch(states, region_size, gen_model2):
	x_pos = np.random.randint(0, 84 - region_size + 1)
	y_pos = np.random.randint(0, 84 - region_size + 1)
	states[0, x_pos: x_pos + region_size, y_pos: y_pos + region_size] = 1.0
	states = torch.tensor(np.float32(states)).unsqueeze(0).to(device)
	states_new = gen_model2(states)
	states[0, 0, x_pos: x_pos + region_size, y_pos: y_pos + region_size] = 1.5
	states_syn = torch.where(states < 1.4, states, states_new)
	states_syn = states_syn.detach().cpu().numpy()
	return states_syn
...
if __name__ == '__main__':
	...
	loadstr = "base2/" + "single_" + load_model_path + '_' + str(0.05)
	...
	for frames in range(num_frames):
		state = mstate_patch(state, patch_size, gen_model2)
