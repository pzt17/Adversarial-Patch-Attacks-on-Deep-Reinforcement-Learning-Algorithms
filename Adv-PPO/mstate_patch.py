...
def mstate_patch(states_ori, region_size, x_pos_list, y_pos_list):
	...
	y_pos_list = y_pos_list.to(device)
	x_pos_list = x_pos_list.to(device)
	y_pos2 = y_pos_list[y_r]
	x_pos2 = x_pos_list[x_r]
	for i in range(half_size):
		states[int(batch_size/2):, x_pos2[i]: x_pos2[i] + region_size, y_pos2[i]:y_pos2[i] + region_size, :] = 255.0
