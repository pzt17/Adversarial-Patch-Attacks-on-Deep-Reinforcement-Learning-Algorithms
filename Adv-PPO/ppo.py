...
class PPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _update(self, dataset):
        ...
        for batch in _yield_minibatches(
            ...
            masked_obs2, x_pos, y_pos, x_pos2, y_pos2= mstate_patch(states, self.patch_size, self.x_pos_list, self.y_pos_list)
            masked_obs = torch.swapaxes(masked_obs2, 1, 3)
            masked_obs = torch.swapaxes(masked_obs, 2, 3)
            gen_obs2 = self.gen_model2(masked_obs)
       	 
            gen_obs2 = torch.swapaxes(gen_obs2, 1, 2)
            gen_obs2 = torch.swapaxes(gen_obs2, 2, 3)
            syn_states = syn_obs(masked_obs2, gen_obs2, self.patch_size, x_pos, y_pos, x_pos2, y_pos2)
            
            mask_logits, _ = self.model(syn_states)
            mask_distribs = torch.distributions.Categorical(logits=mask_logits)
            ...
            loss = self._mask_loss(
                    ...
                    mask_log_probs = mask_distribs.log_prob(actions)
                )
    ...
    def _mask_loss(
        self, entropy, vs_pred, log_probs, vs_pred_old, log_probs_old, advs, vs_teacher, mask_log_probs, kappa=0.5
    ):
        ...
        mask_prob_ratio = torch.exp(mask_log_probs - log_probs_old)
        loss_robust = -torch.mean(
                torch.min(
                    mask_prob_ratio * advs,
                    torch.clamp(mask_prob_ratio, 1 - self.clip_eps, 1 + self.clip_eps) * advs)
        ...
        loss = (
            kappa*loss_policy + (1.0 -kappa)*loss_robust
    ...
    def gau_loss(
        self, entropy, vs_pred, log_probs, vs_pred_old, log_probs_old, advs, vs_teacher, noise_log_probs,  kappa=0.5
    ):
        ...
        noise_prob_ratio = torch.exp(noise_log_probs - log_probs_old)
        loss_robust = -torch.mean(
                torch.min(
                    noise_prob_ratio * advs,
                    torch.clamp(noise_prob_ratio, 1 - self.clip_eps, 1 + self.clip_eps) * advs)
    ...
    def batch_act_logit(self, batch_obs):
       #assert not self.training
       #b_state = self.batch_states(batch_obs, self.device, self.phi)
       b_state = torch.tensor(np.float32(batch_obs), requires_grad = True).to(self.device)
       logits, _ = self.model(b_state)       
       return logits, b_state.grad
       
    def get_q_value(self, batch_obs):
    	#assert not self.training
    	logits, _ = self.model(batch_obs)
    	return logits 
