...
from evaluate import random_patch_evaluate_train
from evaluate import random_patch_evaluate_test
...
if __name__ == '__main__':
    ...
    train_mean, train_std = random_patch_evaluate_train(configs, ppo_agent, train_venv, patch, x_pos, y_pos, gen_model, mask_model)
    eval_mean, eval_std = random_patch_evaluate_test(configs, ppo_agent, valid_venv, patch, x_pos, y_pos, gen_model, mask_model)
    
    file1 = open("random_train_reward.txt", "a")		
    str_txt= "\n" + configs.env_name + " percent image:" + str(configs.percent_image)+ " average reward:" + str(train_mean) + " reward std:" + str(train_std)
    file1.write(str_txt)
    file1.close()
    
    file2 = open("random_eval_reward.txt", "a")		
