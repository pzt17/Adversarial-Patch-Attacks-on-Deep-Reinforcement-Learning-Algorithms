...
def rollout_one_step_oppor(agent, env, obs, steps, env_max_steps=1000, pgd_epsilon=None, gwc_epsilon=None, random_ep = 0.3):
    assert not (pgd_epsilon and gwc_epsilon)
    # Step once.
    if pgd_epsilon:
        action = agent.batch_act_pgd(obs, pgd_epsilon)
    ...
    if random.random() < random_ep:
    	action = np.zeros((1))
    	action[0] = np.int(env.action_space.sample())
    	#print(action)
